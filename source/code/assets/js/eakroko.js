...
$(document).ready(function() {
	var base_url = 'http://localhost/survey/assets/';
	var mobile = false,
	...
	if($('.plupload').length > 0){
		$(".plupload").each(function(){
			...
			$el.pluploadQueue({
				...
				url : base_url+'js/plupload/upload.php',
				max_file_size : '10mb',
				chunk_size : '1mb',
				unique_names : true,
			...
			if(!$(this).hasClass("dataTable-custom")) {
				var opt = {
					...
					"oLanguage":{
					    ...
					    "sLengthMenu":   "_MENU_<span>mục mỗi trang</span>",
				...
				if($(this).hasClass("dataTable-colvis")){
					...
					opt.oColVis = {
						"buttonText": "Thay đổi cột <i class='icon-angle-down'></i>"
					};
				}
				if($(this).hasClass('dataTable-tools')){
					...
					opt.oTableTools = {
						"sSwfPath": base_url+"js/plugins/datatable/swf/copy_csv_xls_pdf.swf"
					};
				}
				if($(this).hasClass("dataTable-scroller")){
					...
					opt.sAjaxSource = base_url+"js/plugins/datatable/demo.txt";
				}
				var oTable = $(this).dataTable(opt);
				$('.dataTables_filter input').attr("placeholder", "nội dung...");
	...
	if($('.file-manager').length > 0)
	{
		$('.file-manager').elfinder({
			url:base_url+'js/plugins/elfinder/php/connector.php'
		});
	}
